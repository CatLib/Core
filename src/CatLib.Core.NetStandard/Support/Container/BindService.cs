...
namespace CatLib
{
    ...
    /// 服务绑定数据
    /// </summary>    
    internal sealed class BindService : IBindService
    {
        /// <summary>
        /// 当前绑定服务的服务名
        /// </summary>
        ...
        /// 服务构造修饰器
        /// </summary>
        private List<Func<IBindService, object, object>> resolving;

        /// <summary>
        /// 服务构造修饰器
        /// </summary>
        private List<Action<IBindService, object>> release;
        ...
        internal BindService(Container container, string service, Func<IContainer, object[], object> concrete, bool isStatic)
        {
        ...
        /// <returns>服务绑定数据</returns>
        public IBindService Alias(string alias)
        {
            lock (syncRoot)
            {
                GuardIsDestroy();
        ...
        /// <returns>服务绑定数据</returns>
        public IBindService OnResolving(Func<IBindService, object, object> func)
        {
            ...
            lock (syncRoot)
            {
                ...
                if (resolving == null)
                {
                    resolving = new List<Func<IBindService, object, object>>();
                }
                resolving.Add(func);
            }
            return this;
        ...
        /// <returns>服务绑定数据</returns>
        public IBindService OnRelease(Action<IBindService, object> action)
        {
            ...
            lock (syncRoot)
            {
                ...
                if (release == null)
                {
                    release = new List<Action<IBindService, object>>();
                }
                release.Add(action);
            }
            return this;
        ...
        /// <returns>服务绑定数据</returns>
        internal BindService AddContextual(string needs, string given)
        {
            lock (syncRoot)
            {
                GuardIsDestroy();
